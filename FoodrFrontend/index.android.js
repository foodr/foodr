...
export default class FoodrFrontend extends Component {
  ...
  searchUPC(upc) {
    ...
    fetch('https://dbc-foodr-api.herokuapp.com/products/upc/' + upc + "?user_id=" + this.state.userId)
  ...
  searchName(name) {
    ...
    fetch('https://dbc-foodr-api.herokuapp.com/products/name/' + name + "?user_id=" + this.state.userId)
  ...
  getProductInfoBySearchID(searchID) {
    ...
    fetch('https://dbc-foodr-api.herokuapp.com/searches/' + searchID)
  ...
  saveSearch(searchId) {
    if (this.state.userId) {
      fetch('https://dbc-foodr-api.herokuapp.com/searches/' + searchId + '/save', {method: 'POST'})
      ...
      .then(jsonData => {
        if (jsonData.save_successful) {
          ...
          Alert.alert('Product was not saved.');
        }
      })
      .catch((error) => {}) // currently not catching errors
    } else {
      Alert.alert('Please sign up or login to save an item.');
    }
  }
  ...
  authenticateUser(email, password) {
    ...
    .then(jsonData => {
      if (jsonData.found) {
        ...
        Alert.alert(jsonData.errors.join("\n"))
      }
    })
    .catch((error) => {}) // currently not catching errors
  }
  ...
  findUser(){
    if (this.state.userId) {
      fetch('https://dbc-foodr-api.herokuapp.com/users/profile/' + this.state.userId)
      ...
      .then(jsonData => {
        ...
        if (this.state.userDetails.found) {
          ...
          Alert.alert('You are not a registered user. Please sign up.');
          this.setState({userId: false})
          this.updateCurrentPage('IndexPage');
        }
      })
      ...
      Alert.alert('Please sign up or login to access your profile.');
    }
  }

  createUser(email, password) {
    fetch('https://dbc-foodr-api.herokuapp.com/users?email=' + email + '&password=' + password, {method: 'POST'})
